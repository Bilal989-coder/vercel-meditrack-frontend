...
const MyAppointments = () => {
    ...
    const [waitlist, setWaitlist] = useState([]) // State to store waitlist data
    ...
    useEffect(() => {
        if (token) {
            getUserAppointments()
            getWaitlist()  // Fetch the waitlist data as well
        }
    }, [token])
        ...
        <div>
            ...
            <div className=''>
                {waitlist.map((item, index) => (
                    <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
                        <div>
                            <img className='w-36 bg-[#EAEFFF]' src={item.doctorData.image} alt="" />
                        </div>
                        <div className='flex-1 text-sm text-[#5E5E5E]'>
                            <p className='text-[#262626] text-base font-semibold'>{item.doctorData.name}</p>
